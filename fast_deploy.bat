...
ssh -o StrictHostKeyChecking=no %DEPLOY_CONNECT% ^
  "docker load -i %APP_NAME%.tar && docker rm -f %APP_NAME% || true && docker run -d --name %APP_NAME% -p 8080:8080 --network %NETWORK% -e VIRTUAL_HOST=\"api.roomily.tech\" -e VIRTUAL_PORT=8080 -e LETSENCRYPT_HOST=\"api.roomily.tech\" -e LETSENCRYPT_EMAIL=\"blog@api.roomily.tech\" %APP_NAME%:%VERSION%"